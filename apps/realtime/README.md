...
The compiled policy is runtime-visible. The backend uses it for resolver follow-up rules, confirmation behavior, escalation keywords, and Realtime session instructions. The browser reads the same policy summary from `/realtime-config` and uses it when asking Realtime to speak each backend-approved reply.
